    ...
    if (leadId) {
        ...
        try {
            ...
            if (ghlResult.ghlContactId) {
                ...
                if (updateError) {
                    console.error('[lead-actions] Failed to save GHL ID to lead:', updateError)
                } else {
                    console.log(`[lead-actions] Successfully linked Lead ${leadId} to GHL Contact ${ghlResult.ghlContactId}`)
                }
            }
        } catch (error) {
...
export async function getLeads() {
    ...
    const permission = userAccess.permissions.can_view_leads
    ...
    // If 'own' scope, filter by assignment
    if (permission === 'own') {
        const userId = (await supabase.auth.getUser()).data.user?.id
        if (userId) {
            query = query.or(`assigned_user_id.eq.${userId},booked_by_user_id.eq.${userId}`)
        }
    }
...
// Cached functions moved to @/lib/actions/lead-analytics.ts
